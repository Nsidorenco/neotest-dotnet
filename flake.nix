{
  ...
  outputs =
    inputs@{ self, nixpkgs, flake-parts, pre-commit-hooks, neorocks, ... }:
    let
      name = "neotest-dotent";
    ...
    in flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { config, self', inputs', system, ... }:
        let
          ...
          pkgs = import nixpkgs {
            inherit system;
            overlays = [ ci-overlay neorocks.overlays.default plugin-overlay ];
          };

          mkTypeCheck = { nvim-api ? [ ], disabled-diagnostics ? [ ], }:
                ...
                lua-ls = {
                  config = {
                    ...
                    Lua = {
                      workspace = {
                        library = nvim-api
                          ++ [ (with pkgs.vimPlugins; [ neotest ]) ] ++ [
                            "\${3rd}/busted/library"
                            "\${3rd}/luassert/library"
                          ];
                        ignoreDir =
                          [ ".git" ".github" ".direnv" "result" "nix" "doc" ];
                      };
                      diagnostics = {
